// app.js - MLM Dashboard v9 (Zoom + Scroll perbaikan final)
...
auth.onAuthStateChanged(user => {
  if (user) {
    q("loginSection").style.display = "none";
    q("mainSection").style.display = "block";
    q("logoutBtn").style.display = "inline-block";
    loadMembers();
  } else {
    q("mainSection").style.display = "none";
    q("loginSection").style.display = "block";
...
q("memberForm").onsubmit = e => {
  ...
  if (!data.nama || !data.uid) return alert("Nama dan UID wajib diisi!");
...
// ===== NETWORK TREE (Zoom & Scroll diperbaiki) =====
function renderNetwork() {
  ...
  // ==== FIX: Zoom & Pan aktif di semua device ====
  const svgNode = svg.node();
  svgNode.style.touchAction = "none"; // aktifkan gesture mobile
